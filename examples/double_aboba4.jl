...
@printf("f₂ estimate with %d samples: %g\n", length(Xvals), f₂_estimate);
Random.seed!(100);
observable_samples = sample_observables(ComplexF64, [q₀, p₀], sampler, observables, options=MDOptions(n_iters=n_iters,n_save_iters=n_save_iters));
@printf("f₁ estimate with %d samples: %g + i%g\n",length(observable_samples), 
    mean(real.(observable_samples[1,:])),mean(imag.(observable_samples[1,:])));
@printf("f₂ estimate with %d samples: %g + i%g\n", length(observable_samples), 
    mean(real.(observable_samples[2,:])),mean(imag.(observable_samples[2,:])));
